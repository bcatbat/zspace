import { loadingPage } from './zzUtils';
import * as utils from './zzUtils';
			...
			const asset_1: cc.Asset[] = await new Promise((resolveFn, rejectFn) => {
				bundle.loadDir(
					...
					(finish: number, total: number) => {
						option && option.showLoading && loadingPage(true, loadingLmtD + (finish / total) * (loadingLmtU - loadingLmtD), '加载资源');
					},
					(err, res: cc.Asset[]) => {
