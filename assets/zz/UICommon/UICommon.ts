...
import { event, UIBase } from '../zz';

const { ccclass, property } = cc._decorator;

@ccclass
export default class UICommon extends UIBase {
	onLoad() {
		this.InitLabelTip();
		this.InitLoadNode();
		event.register(EventType.Tip, this, this.ShowLabelTip);
		event.register(EventType.LoadingPage, this, this.LoadNodeMgr);
		this.node.zIndex = 100;
	}
	onDestroy() {
		event.delAllRegister(this);
	}
	...
	InitLoadNode() {
		...
		this.valLabel = this.loadNode.children[1].children[0].getComponent(cc.Label);
		this.desLabel = this.loadNode.children[1].children[1].getComponent(cc.Label);
	}
