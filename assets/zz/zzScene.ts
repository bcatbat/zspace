...
import { Dictionary } from './zzStructure';
import { getBundle, instantiatePrefab } from './zzUtils';
...
export class SceneMgr {
	/**已显示的场景节点字典; */
	private sceneDict: Dictionary<string, cc.Node> = new Dictionary<string, cc.Node>();
	/**预载场景节点字典;未显示 */
	private preloadDict: Dictionary<string, cc.Node> = new Dictionary<string, cc.Node>();
	/**加载标记;防止重复加载 */
	private loadingDict: Dictionary<string, number> = new Dictionary<string, number>();
	/**打开中标记;用于预载过程中打开 */
	private openningDict: Dictionary<string, number> = new Dictionary<string, number>();
	...
	public async loadScene(sceneName: string, bundleName: string) {
		...
		try {
			...
			let node = await instantiatePrefab(prefab_1);
			this.sceneRoot.addChild(node);
