...
export class ResMgr {
	...
	async loadResDict(
		...
		try {
			...
			option && option.showLoading && utils.loadingPage(true, loadingLmtD, '加载资源');
			let bundle = await utils.getBundle(bundleName);
			const asset_1: cc.Asset[] = await new Promise((resolveFn, rejectFn) => {
				bundle.loadDir(
					dirName,
					(finish: number, total: number) => {
						option && option.showLoading && utils.loadingPage(true, loadingLmtD + (finish / total) * (loadingLmtU - loadingLmtD), '加载资源');
					},
					(err, res: cc.Asset[]) => {
						option && option.closeLoadingOnFinish && utils.loadingPage(false, 100, '');
