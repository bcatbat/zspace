...
/**长屏判断 */
export function isLongScreen() {
	return cc.winSize.height / cc.winSize.width > 2;
}

/**弹出节点 */
export function zoomOutNode(node: cc.Node, fromScale = 0.5, toScale = 1, time = 0.2) {
    //TEMP 弹出节点与新手引导相冲
	return;
	node.scale = fromScale;
	node.runAction(cc.scaleTo(time, toScale));
}

/**数字缩写,3位有效数字 */
export function formatNumZh3(num: number) {
	if (!num) return '0';
	if (num < 1e4) {
		return num.toFixed(0);
	} else if (num < 1e8) {
		return (num / 1e4).toPrecision(3) + '万';
	} else {
		return (num / 1e8).toPrecision(3) + '亿';
	}
}

/**设置节点触摸事件是否吞没 */
export function setSwallowTouch(node: cc.Node, isSwallow: boolean) {
	(node as any)._touchListener.setSwallowTouches(isSwallow);
}
