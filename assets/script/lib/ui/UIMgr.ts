import { warn, error } from '../log/Log';
import { UIBase } from './UIBase';
import { loadingPage, instantiatePrefab, getBundle } from '../utils/Utils';
...
export class UIMgr {
	...
	async openUI(uiArgs: UIArgs) {
		...
		try {
			...
			let uiNode: cc.Node = await instantiatePrefab(prefab_1);
	...
	private async getUIBundle(uiName: string) {
		...
		let bundle = await getBundle(path);
		return bundle;
	}
	/**从场景中移除UI; 保留本地缓存; */
